...
Thread t;
EventQueue store_queue(32 * EVENTS_EVENT_SIZE);
Thread store_t;
...
RPCFunction rpcStatus(&getStatus, "getStatus");
...
int main(){
  ...
  store_t.start(callback(&store_queue, &EventQueue::dispatch_forever));

  // Setup a serial interrupt function of receiving data from xbee
  xbee.attach(xbee_rx_interrupt, Serial::RxIrq);

  while (true) {
    store_queue.call(&getAcc);
    wait(0.1);
  }
}
